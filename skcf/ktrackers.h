...
class KTrackers {
  public:
    ...
    void set_area(const cv::Rect &rect)
    {
        ...
        setArea(RotatedRect(center,targetSize,0));
    }

    cv::Rect get_area(const cv::Mat& frame) {
        processFrame(frame);
  ...
  private:
    ...
    static void gaussian_shaped_labels(float sigmaW, float sigmaH, const Size& sz,
                                       Mat& labels);
